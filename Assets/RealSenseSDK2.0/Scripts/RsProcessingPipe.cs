...
public class RsProcessingPipe : RsFrameProvider
{
    ...
    internal void ProcessFrame(Frame frame, FrameSource src)
    {
        try
        {
            ...
            Frame f = frame;// the temporary frame we're working with right now

            if (profile != null)//if there is a profile, do stuff
            {
                var filters = profile._processingBlocks.AsReadOnly(); //get a list of all the processingblocks from profile

                foreach (var pb in filters) //loop through
                {
